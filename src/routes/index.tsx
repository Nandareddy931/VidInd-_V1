...
function HomePage() {
  ...
  return (
    <AppLayout>
      {/* Hero banner — tablet & desktop */}
      <section className="hidden md:block relative overflow-hidden rounded-2xl lg:rounded-3xl mb-6 lg:mb-8 shadow-elevated animate-fade-in">
        <img src={heroBanner} alt="" className="absolute inset-0 h-full w-full object-cover opacity-60" />
        <div className="absolute inset-0 bg-gradient-to-r from-background via-background/70 to-transparent" />
        <div className="relative px-6 md:px-8 lg:px-10 py-8 md:py-10 lg:py-14 max-w-2xl">
          ...
          <h1 className="mt-3 lg:mt-4 text-2xl md:text-3xl lg:text-5xl font-extrabold tracking-tight leading-tight">
            Discover. Watch. <span className="gradient-text">Create. Grow.</span>
          </h1>
          <p className="mt-2 lg:mt-3 text-sm md:text-base text-muted-foreground max-w-md">
          ...
          <div className="mt-4 lg:mt-6 flex flex-wrap gap-2 lg:gap-3">
            <Link
              to="/upload"
              className="inline-flex items-center gap-2 rounded-full bg-gradient-primary px-4 lg:px-5 py-2 lg:py-2.5 text-sm font-semibold text-white glow-primary hover:opacity-90 transition-smooth"
            >
              <PlayCircle className="h-4 w-4" /> Start creating
            </Link>
            <Link
              to="/trending"
              className="inline-flex items-center gap-2 rounded-full glass px-4 lg:px-5 py-2 lg:py-2.5 text-sm font-semibold text-foreground hover:border-accent transition-smooth"
            >
      ...
      <section className="mt-6 lg:mt-8">
        ...
        {loading ? (
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 lg:gap-5">
